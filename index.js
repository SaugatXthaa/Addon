...
const manifest = {
    ...
    types: ["movie", "series"],
    idPrefixes: ["tt"],
    catalogs: [],           // ← This line fixes the error
...
async function scrapeSite(imdbId, site) {
    ...
    // Basic regex to find stream links
    const urlRegex = /(https?:\/\/[^\s"']+\.(mp4|m3u8|embed)[^\s"']*)/gi;
    let match;
    while ((match = urlRegex.exec(html)) !== null) {
        const link = match[1];
        const lower = link.toLowerCase();
        let quality = "1080p";
        if (lower.includes("2160") || lower.includes("4k") || lower.includes("uhd")) quality = "4K";
...
app.listen(PORT, () => {
    console.log(`✅ Filmxy Multi Addon running on port ${PORT}`);
});
